ServerEvents.commandRegistry((event) => {
  ...
  event.register(
    Commands.literal("create_manual_config")
      .then(
        Commands.literal("page_type")
          .then(
            Commands.literal("PonderTagIndexScreen").executes((ctx) => {
              let player = ctx.source.player;
              player.persistentData.create_manual_page_type =
          ...
          .then(
            Commands.literal("PonderIndexScreen").executes((ctx) => {
              let player = ctx.source.player;
              player.persistentData.create_manual_page_type =
                "PonderIndexScreen";
      ...
      .then(
        Commands.literal("default_page_type")
          ...
          .then(
            Commands.literal("PonderTagIndexScreen").executes((ctx) => {
              ...
              player.tell(
                "Now create manual will open PonderTagIndexScreen by default!",
              );
              return 1;
            }),
          )
          .then(
            Commands.literal("PonderIndexScreen").executes((ctx) => {
              ...
              player.tell(
                "Now create manual will open PonderIndexScreen by default!",
              );
              return 1;
            }),
          ),
      ...
      .then(
        Commands.literal("help").executes((ctx) => {
          let player = ctx.source.player;
          player.tell(
            "/create_manual_config default_page_type <type> - will set default screen type for all players.\n" +
