function get_page_type(ponder, player) {
  if (!player.persistentData.contains("create_manual_page_type")) {
    if (
      !("page_type" in ponder) ||
      ponder.get("page_type") == "PonderTagIndexScreen"
    ) {
...
function create_manual_right_click(event) {
  if (event.player.mainHandItem == "create:manual") {
    ...
    event.server.scheduleInTicks(3, (callback) => {
      players_create_manual_right_click =
        players_create_manual_right_click.filter(
    ...
    if (ray.block !== null && !event.player.isCrouching()) {
      ...
      if (
        ponders.includes("create:vertical_gearbox") &&
        ponders.includes("create:gearbox")
      ) {
    ...
    if (page_type == "PonderIndexScreen") {
      event.server.runCommandSilent(
        `/execute as ${event.entity.uuid} run create ponder`,
      );
      return;
    }
  }
...
NetworkEvents.fromClient("create_manual_change_screen", (event) => {
  let screen = null;
  if (
    !event.player.persistentData.contains("create_manual_page_type") ||
    event.player.persistentData.create_manual_page_type ==
      "PonderTagIndexScreen"
  ) {
    screen = "PonderIndexScreen";
  } else if (
    event.player.persistentData.create_manual_page_type == "PonderIndexScreen"
  ) {
